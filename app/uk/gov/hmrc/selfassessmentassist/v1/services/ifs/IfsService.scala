...
import uk.gov.hmrc.selfassessmentassist.utils.{CurrentDateTime, Logging}
import uk.gov.hmrc.selfassessmentassist.v1.models.domain.{AssessmentReport, AssessmentRequestForSelfAssessment, Link, PreferredLanguage, Risk}
import uk.gov.hmrc.selfassessmentassist.v1.services.ifs.models.request.{IFRequest, IFRequestPayload, IFRequestPayloadAction, IFRequestPayloadActionLinks, Messages}
...
class IfsService @Inject()(connector: IfsConnector, currentDateTime: CurrentDateTime) extends Logging {
  ...
  private def risks(report: RdsAssessmentReport, preferredLanguage: PreferredLanguage): Seq[Risk] = {
    report.outputs.collect {
      ...
      case value: RdsAssessmentReport.DataWrapper => value
    }.flatMap(_.data.getOrElse(Seq.empty)).flatMap(toRisk)
  }
