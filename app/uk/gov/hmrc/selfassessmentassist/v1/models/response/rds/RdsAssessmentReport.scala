...
case class RdsAssessmentReport(links: Seq[String], version: Int, moduleId: String, stepId: String, executionState: String, outputs: Seq[Output]) {

  def calculationId: Option[UUID] = outputs.collectFirst { case KeyValueWrapper("calculationId", value) =>
    value.map(UUID.fromString)
  }.flatten

  def rdsCorrelationId: Option[String] = outputs.collectFirst { case KeyValueWrapper("correlationId", value) =>
    value
  }.flatten

  def feedbackId: Option[UUID] = outputs.collectFirst { case KeyValueWrapper("feedbackId", value) =>
    value.map(UUID.fromString)
  }.flatten

  def responseCode: Option[Int] = outputs.collectFirst { case KeyValueWrapperInt("responseCode", value) =>
    value
  }

  def responseMessage: Option[String] = outputs.collectFirst { case KeyValueWrapper("responseMessage", value) =>
    value
  }.flatten

  def calculationTimestamp: Option[String] = outputs.collectFirst { case KeyValueWrapper("calculationTimestamp", value) =>
    value
  }.flatten
...
object RdsAssessmentReport {
  ...
  object Output {
    ...
    val specialKeys: Seq[String] = List(
      "correlationId",
      "feedbackId",
      "calculationId",
      "nino",
