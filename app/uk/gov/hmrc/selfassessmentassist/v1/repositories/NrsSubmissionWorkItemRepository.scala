    ...
    collection
      .deleteOne(
        filter = Filters.and(
          ...
          Filters.equal(workItemFields.status, ProcessingStatus.Succeeded)
        )
      )
      .toFuture()
      .map(_.getDeletedCount > 0)

  def findSucceededItems(): Future[Seq[WorkItem[NrsSubmissionWorkItem]]] =
    collection.find(Filters.equal(workItemFields.status, ProcessingStatus.Succeeded)).toFuture()
