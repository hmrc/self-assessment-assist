...
class AssessmentRequestForSelfAssessmentSpec extends  UnitSpec {
  ...
  val assessmentReportModelJson: JsValue = Json.parse(
    s"""
       ...
       |"customer_type":"T",
       |"agent_ref": "$agentRef",
       |"tax_year": "${assessmentRequestForSelfAssessmentModel.taxYear}"
       |}
       |""".stripMargin)

  "AssessmentRequestForSelfAssessment" when {
    "read to JSON" must {
      "product the specific model" in {
        assessmentReportModelJson.as[AssessmentRequestForSelfAssessment] shouldBe assessmentRequestForSelfAssessmentModel
      }
    }
  }
}
