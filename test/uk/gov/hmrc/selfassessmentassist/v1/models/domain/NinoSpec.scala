...
package uk.gov.hmrc.selfassessmentassist.models.domain
...
class NinoSpec extends UnitSpec {
  ...
  "Creating a Nino" should {
    "fail if the nino is not valid" in {
      an[IllegalArgumentException] should be thrownBy NinoChecker("INVALID_NINO")
    }
  }

  "Formatting a Nino" should {
    "produce a formatted nino" in {
      NinoChecker("CS100700A").formatted shouldBe "CS 10 07 00 A"
    }
  }

  "Removing a suffix" should {
    "produce a nino without a suffix" in {
      NinoChecker("AA111111A").withoutSuffix shouldBe "AA111111"
    }
  }

  def validateNino(nino: String): Boolean = NinoChecker.isValid(nino)
}
